...
<li class="down-arrow custom-tooltip-assignment col-1 list-inline-item icon-padding">
<div class="tooltiptext">Assignment</div>
<img src="static/images/task.png" style="width:30px" height="30px">
${(() => {
...
<li class="custom-tooltip-notes down-arrow2 col-1 list-inline-item icon-padding">
<div class="tooltiptext">Notes</div>
<img src="static/images/borrow-book.png" style="width:30px" height="29px">
${(() => {
...
function getUploadSignedURL(
    ...
    $.ajax({
        ...
        xhr.setRequestHeader('Content-Length', x.files[0].size)
        var fd = new FormData()
        ...
        $.ajax({
            ...
            axios({
                ...
                onDownloadProgress: (progressEvent) => {
                    ...
                    if (totalLength !== null) {
                        const percent = Math.round((progressEvent.loaded * 100) / totalLength);
                        lecture.find('.progress').removeClass('d-none')
                        lecture.find('.progress-bar').css('width', `${percent}%`)
                    }
                }
            }).then((response) => {
                $.ajax({
