...
function getAssignmentGreenDotNoOrangeDot() {
    return `<sup><i class="fa fa-circle assignment-submitted-dot"></i><i class="fa fa-circle assignment-available-dot d-none"></i></sup>`
}

function getAssignmentNoGreenDotOrangeDot() {
    return `<sup><i class="fa fa-circle assignment-submitted-dot d-none"></i><i class="fa fa-circle assignment-available-dot"></i></sup>`
}

function getAssignmentNoGreenDotNoOrangeDot() {
    return `<sup><i class="fa fa-circle assignment-submitted-dot d-none"></i><i class="fa fa-circle assignment-available-dot d-none"></i></sup>`
}
        ...
        for(var i = 0; i < lectures.length; i++) {
            ...
            ${(() => {
              ...
              <div class="action d-none">
                ...
                <div class="view-comments view-comment-assignment">View Comments<i class="fa fa-caret-down pl-2 pr-2" aria-hidden="true"></i></div>
              ...
              <div class="action2 d-none">
                ...
                <div class="view-comments view-comment-notes">View Comments<i class="fa fa-caret-down pl-2 pr-2" aria-hidden="true"></i></div>
              ...
              <ul class="list-inline mt-2 d-flex container-fluid">
                <li class="col-10 list-inline-item m-0 underline">
                  <input class="form-control form-control-sm comment-input" type="text" placeholder="Add a comment...">
                </li>
                <li class="col-1 list-inline-item m-0 underline">
                  <div class="character-count">0/140</div>
                </li>
    ...
    $('.view-comments').click(function(){
        ...
        $.ajax({
            ...
            if(data.comments.length == 0){
                console.log('yes')
                comments_box.prepend(`<div id="no-comments-yet">No comments yet</div>`)
            }
            else
...
function drawDays() {
    ...
    days.forEach(day => {
        ...
        if(day.currentMonth && calendar_notifications.has(dateNotification)){
            daysTemplate += `
            <li class="${day.currentMonth ? 'custom-tooltip-2 ' : 'another-month'}${day.today ? ' active-day ' : ''}${day.selected ? 'selected-day' : ''}" data-day="${day.dayNumber}" data-month="${day.month}" date-notification="yes" data-year="${day.year}"><div class="dot"></div>${day.currentMonth ? '<span class="tooltiptext-2">Tooltip text</span>' : ''}</li>
            `
        }
        else daysTemplate += `
        <li class="${day.currentMonth ? 'custom-tooltip-2 ' : 'another-month'}${day.today ? ' active-day ' : ''}${day.selected ? 'selected-day' : ''}" data-day="${day.dayNumber}" data-month="${day.month}" date-notification="yes" data-year="${day.year}">${day.currentMonth ? '<span class="tooltiptext-2">Tooltip text Tooltip text</span>' : ''}</li>
        `
    });
